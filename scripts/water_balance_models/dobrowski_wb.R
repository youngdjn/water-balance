...
monthlyET0 <- function(radiation,rad.nldas,tmax,tmin,wind,lat,elev,dpt,tmean_prev,albedo=0.23,month){
...
dobr_wb <- function(snowpack_prev_init,tmean_prev_init,soil_prev_init,monthlist,T.m,P.m,R.m,R.nldas.m,Tmin.m,Tmax.m,wind.m,Tdew.m,L,E,S.max,pet.mult) {
  ...
  for(i in 1:ncol(T.m)) {
    ...
    input.df <- snowmod(tmean=T.m[,i],ppt=P.m[,i],radiation=R.m[,i],snowpack_prev=snowpack_prev_cur,albedo=0.23,albedo_snow=0.80) #snow was 0.8
    input[,i] <- input.df$input
    pet[,i] <- monthlyET0(radiation=R.m[,i],rad.nldas=R.nldas.m[,i],tmax=Tmax.m[,i],tmin=Tmin.m[,i],wind=wind.m[,i],lat=L,elev=E,dpt=Tdew.m[,i],tmean_prev=tmean_prev_cur,albedo=input.df$albedo,month=month) * pet.mult
  ...
  wb <- list(aet=aet,def=def,pet=pet)
...
run_dob_wb <- function(T.m,P.m,R.m,R.nldas.m,Tmin.m,Tmax.m,wind.m,Tdew.m,L,E,S.max,pet.mult=1) {
  ...
  R.nldas.m <- cbind(R.nldas.m,R.nldas.m,R.nldas.m)
  ...
  wb <- dobr_wb(snowpack_prev_init,tmean_prev_init,soil_prev_init,monthlist,T.m,P.m,R.m,R.nldas.m,Tmin.m,Tmax.m,wind.m,Tdew.m,L,E,S.max,pet.mult)
